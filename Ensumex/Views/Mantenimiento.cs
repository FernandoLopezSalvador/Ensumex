...
namespace Ensumex.Views
{
    public partial class Mantenimiento : UserControl
    {
        ...
        private void ConfigurarGrid()
        {
            ...
            if (!dgvMantenimiento.Columns.Contains("ModificarNumero"))
            {
            ...
            if (!dgvMantenimiento.Columns.Contains("FrecuenciaCombo"))
            {
                ...
                comboCol.Items.AddRange(6, 12, 18, 24, 36); 
        ...
        private void dgvMantenimiento_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0 && dgvMantenimiento.Columns[e.ColumnIndex].Name == "ModificarNumero")
            {
                var row = dgvMantenimiento.Rows[e.RowIndex];
                int id = Convert.ToInt32(row.Cells["Id"].Value);
                string numeroActual = row.Cells["Telefono"].Value?.ToString() ?? "";

                using (var form = new Form())
                {
                    form.Text = "Modificar Número";
                    ...
                    var modificarNumeroUC = new ModificarNumeroForm(numeroActual);
                    modificarNumeroUC.Dock = DockStyle.Fill;
                    form.Controls.Add(modificarNumeroUC);

                    modificarNumeroUC.NumeroGuardado += (s, args) => form.DialogResult = DialogResult.OK;

                    if (form.ShowDialog() == DialogResult.OK)
                    {
                        SqlServerRepository.ActualizarTelefonoCliente(id, modificarNumeroUC.NuevoNumero);
                        row.Cells["Telefono"].Value = modificarNumeroUC.NuevoNumero;
        ...
        private void dgvMantenimiento_CurrentCellDirtyStateChanged(object sender, EventArgs e)
        {
            if (dgvMantenimiento.IsCurrentCellDirty &&
                dgvMantenimiento.CurrentCell is DataGridViewComboBoxCell)
            {
