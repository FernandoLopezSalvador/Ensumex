    ...
    internal class PDFGenerator
    {
            ...
            DataGridView tablaCotizacion)
        {
            try
            {
                ...
                // Totales
                tabla.AddCell(new PdfPCell(new Phrase("Descuento", fontNormal)) { Colspan = 3, HorizontalAlignment = Element.ALIGN_RIGHT });
                tabla.AddCell(new Phrase(descuento, fontNormal));

                tabla.AddCell(new PdfPCell(new Phrase("Subtotal", fontNegrita)) { Colspan = 3, HorizontalAlignment = Element.ALIGN_RIGHT });
                tabla.AddCell(new Phrase(subtotal, fontNormal));

                tabla.AddCell(new PdfPCell(new Phrase("MANO DE OBRA POR INSTALACIÓN", fontNormal)) { Colspan = 3, HorizontalAlignment = Element.ALIGN_RIGHT });
                tabla.AddCell(new Phrase(costoInstalacion, fontNormal));

                tabla.AddCell(new PdfPCell(new Phrase("Costo por Envio/Flete", fontNormal)) { Colspan = 3, HorizontalAlignment = Element.ALIGN_RIGHT });
                tabla.AddCell(new Phrase(costoFlete, fontNormal));

                tabla.AddCell(new PdfPCell(new Phrase("Total", fontNegrita)) { Colspan = 3, HorizontalAlignment = Element.ALIGN_RIGHT });
                ...
                doc.Add(new Paragraph("Atentamente,\nMario Valdez\nRepresentante de Ventas", fontNormal) { Alignment = Element.ALIGN_CENTER });
