...
namespace Ensumex.Services
{
    public static class SincronizacionService
    {
        public static void SincronizarDatos(Action<int, int> actualizarProgreso)
        {
            ...
            DataTable mantenimientos = FirebirdRepository.GetCalentadoresParaMantenimiento();

            int total = productos.Rows.Count + clientes.Rows.Count + precios.Rows.Count + mantenimientos.Rows.Count;
            int progreso = 0;
            ...
            using (SqlConnection conn = SqlServerRepository.GetConnection())
            {
                ...
                using (SqlTransaction transaction = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        foreach (DataRow row in mantenimientos.Rows)
                        {
                            SqlServerRepository.InsertarMantenimiento(row, conn, transaction);
                            progreso++;
                            actualizarProgreso(progreso, total);
                        }
